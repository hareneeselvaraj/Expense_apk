    ...
    <div ref={containerRef} style={{ position: "relative", width: "100%" }}>
      ...
      <div onClick={() => setIsOpen(!isOpen)} style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap:12 }}>
          {selectedColor() ? (
             <div style={{ width: 10, height: 10, borderRadius: "50%", background: selectedColor(), boxShadow: `0 0 10px ${selectedColor()}44` }} />
          ) : selectedEmoji() ? (
             <span style={{ fontSize: 16 }}>{selectedEmoji()}</span>
          ...
          <div style={{display:"flex", flexDirection:"column"}}>
            <span style={{ color: (multi ? value?.length > 0 : value) ? C.text : C.sub, fontSize: 13, fontWeight: 700 }}>{selectedText()}</span>
          </div>
        </div>
